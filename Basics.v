...
(** **** Exercise: 1 star, standard (ltb)  
    ...
    yielding a [b]oolean.  Instead of making up a new [Fi xpoint] for
...
Fixpoint Simple_Collatz (b : bin) : bin :=
  match b with
    | BO => BO
    | D b' => Simple_Collatz b'
    | M b' =>
      match b' with
        | BO => b
        | D b'' => Simple_Collatz (incbin(b))
        | M b'' => Simple_Collatz(incbin(b))
      end
  end.
...
Fixpoint bin_to_nat (m:bin) : nat :=
  match n with
    | Z => O
    | A x => twice_nat (bin_to_nat x)
    | B x => S (twice_nat (bin_to_nat x))
end.
